defmodule AshIntro.ActorPersistor do
  use AshOban.ActorPersister

  def store(%AshIntro.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(AshIntro.Accounts.User, id, authorize?: false) do
